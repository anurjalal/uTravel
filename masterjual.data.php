...
<tbody>
    <?php 
        ...
        $jml_data = mysql_num_rows(mysql_query("SELECT * FROM hrgjual"));
        ...
        if(isset($_POST['cari'])) {
            ...
            $query = mysql_query("
                ...
                OR hargajual like '%$kunci%'
            ");
        // query jika nomor halaman sudah ditentukan
        } elseif(isset($_POST['halaman'])) {
            $halaman = $_POST['halaman'];
            $i = ($halaman - 1) * $jml_per_halaman  + 1;
            $query = mysql_query("SELECT * FROM hrgjual LIMIT ".(($halaman - 1) * $jml_per_halaman).", $jml_per_halaman");
        // query ketika tidak ada parameter halaman maupun pencarian
        } else {
            $query = mysql_query("SELECT * FROM hrgjual LIMIT 0, $jml_per_halaman");
            $halaman = 1; //tambahan
        }
    ...
    <tr>
		...
		<td><?php echo number_format($data['hargajual'],0,',','.') ?></td>
        <td>
            <a href="edit_hargajual.php?id=<?php echo $data['kd_hrgjual']?>" id="<?php echo $data['kd_hrgjual']?>" class="ubahhargajual" data-toggle="modal">
