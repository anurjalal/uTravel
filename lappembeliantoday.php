...
<head>
<font face="courier" size=3><b>LAPORAN PEMBELIAN HARI INI</b></font>
<br> <br>
<?php

echo "<font face=\"courier\" size=3><b> Tanggal : ".date("d-M-Y")."</b></font>";
...
$myquery1="SELECT * FROM pembelian where pembelian.tgl_po between '$today' and '$today2' order by pembelian.nama_supplier";
$myresult1=mysql_query($myquery1);
$datapurchase1=mysql_fetch_array($myresult1);
//$myquery2="SELECT sum(total_harga) as total_harga, sum(pembulatan) as pembulatan, sum(nominal_yg_dibayar) as nominal_yg_dibayar FROM pembelian_detail where pembelian_detail.tgl_po between '$today' and '$today2' order by pembelian_detail.nama_supplier";
